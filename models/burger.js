...
//Here, instead of creating our tables seperately in a sql file, we are going to let sequelize do that for us. We are going ot set it up for it to create everything for us when the server is started. We are going to let the Sequelize ORM create the id for us (thank you), we will create the column name for the burger name, we will create the devoured column and set the value to false, and we will let the sequelize orm also create a timestamp for us. (thank you)
